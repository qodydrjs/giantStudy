...
public class PageVO {
	    ...
	    private void makePaging() {
	        ...
	        int startPage = ((pageNo - 1) / 3) * 3 + 1; // 시작 페이지 (페이징 네비 기준)
	        int endPage = startPage + 3 - 1; // 끝 페이지 (페이징 네비 기준)
	        ...
	        if (isNowFirst) {
	            ...
	            this.setPrevPageNo(((pageNo-1)/blockSize*blockSize)+1); // 이전 페이지 번호
	        }
	        ...
	        if (isNowFinal) {
	            ...
	            this.setNextPageNo(((pageNo-1)/blockSize*blockSize)+blockSize); // 다음 페이지 번호
	        }
